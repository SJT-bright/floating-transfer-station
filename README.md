...
5. 功能按钮和右上角拖动图标支持悬停高亮、轻微上浮与阴影，移开后平滑回落；拖动窗口时图标持续高亮。禁用按钮不响应悬停。系统开启“减少动态效果”时只保留高亮，不进行缩放和位移。
6. 外观设置新增“磨砂强度”和“液态玻璃强度”，分别在 0%—100% 调节材质混合程度，0% 关闭；不会模糊文字和图片。macOS 26 使用原生 Liquid Glass，旧系统使用兼容材质与高光边缘；强度不是系统模糊半径。降低背景不透明度可让材质更明显。升级后默认关闭新增效果以保留原外观，设置实时保存；系统开启“降低透明度”时暂停材质效果。

macOS 批量清空只删除当前分类中未置顶的内容，置顶的文字、图片和文件均保留；全部置顶时清空按钮禁用。单条删除仍可删除置顶项，其他分类不受影响。

分类栏从上到下先显示“待分类”，再显示人物资产、场景、提示词和自定义分类。顶部搜索框按名称检索全部分类；卡片点击铅笔可命名，已命名的分类文字默认只显示名称，点击名称展开正文。收回动画短时缓存面板绘制结果，结束后恢复实时绘制。
...
脚本会生成并打开 `macos/.build/release/悬浮中转站.app`。窗口使用跟随系统深浅色与“降低透明度”辅助设置的原生半透明材质；按住右上角拖动图标可在屏幕内自由移动，距离左右边缘约 20 点时自动吸附，拉开即可脱离。自由悬浮时保持展开；吸边后鼠标移入展开、离开收回，左右两侧都支持。位置自动保存并在重启后恢复；其他区域不会拖动窗口本体。直接拖动卡片里的图片即可把图片拖到其他应用；也可以把图片拖到右侧的其他分类，目标分类会收到独立副本，原分类中的图片保持不动。拖拽期间面板会保持展开；应用只会通过菜单中的“退出悬浮中转站”主动退出。应用还使用 macOS 原生登录项在用户登录时自动启动，并支持文字与图片剪贴板自动采集、外部拖入、“人物资产”“场景”“提示词”和“待分类”四个分类、分类改名、置顶、跨分类移动、复制、删除、清空、悬浮置顶和本地持久化。数据默认保存在 `~/Library/Application Support/悬浮中转站/Data/`；拖出的图片会在 `~/Movies/悬浮中转站素材/` 保留兼容副本，剪映项目仍在使用素材时请勿删除该目录。

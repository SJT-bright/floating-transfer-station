...
4. 展开后点击标题栏的半黑半白圆形按钮进入“外观设置”，分别调节文字深浅、文字不透明度、背景深浅和背景不透明度。实时生效并自动保存；“恢复系统默认”只重置外观。系统开启“降低透明度”时面板背景遵循辅助设置。
5. 功能按钮和底部“按住拖动”区域支持悬停高亮、轻微上浮与阴影，移开后平滑回落；拖动窗口时把手持续高亮，松开后恢复悬停状态。禁用按钮不响应悬停。系统开启“减少动态效果”时只保留高亮，不进行缩放和位移。
...
脚本会生成并打开 `macos/.build/release/悬浮中转站.app`。窗口使用跟随系统深浅色与“降低透明度”辅助设置的原生半透明材质；按住右侧竖栏可在屏幕内自由移动，距离左右边缘约 20 点时自动吸附，拉开即可脱离。自由悬浮时保持展开；吸边后鼠标移入展开、离开收回，左右两侧都支持。位置自动保存并在重启后恢复；其他区域不会拖动窗口本体。直接拖动卡片里的图片即可把图片拖到其他应用；也可以把图片拖到右侧的其他分类，目标分类会收到独立副本，原分类中的图片保持不动。拖拽期间面板会保持展开；应用只会通过菜单中的“退出悬浮中转站”主动退出。应用还使用 macOS 原生登录项在用户登录时自动启动，并支持文字与图片剪贴板自动采集、外部拖入、“人物资产”“场景”“提示词”和“待分类”四个分类、分类改名、置顶、跨分类移动、复制、删除、清空、悬浮置顶和本地持久化。数据默认保存在 `~/Library/Application Support/悬浮中转站/Data/`；拖出的图片会在 `~/Movies/悬浮中转站素材/` 保留兼容副本，剪映项目仍在使用素材时请勿删除该目录。

macOS 收集规则：复制的文字和图片始终自动进入“待分类”，查看其他类别或鼠标离开面板不会改变接收位置。只有主动拖入、拖动归类或使用卡片的“移动到”菜单才会放入其他类别；已有内容不迁移。右上角“＋”可新增自定义类别（名称最多 6 个字符），分类栏支持滚动查看，新增类别、名称和内容都会保存在本机。

macOS 1.5.0 新增固定的“文件中转站”：右侧分类栏底部常驻显示，不随其他分类滚动。点击后使用“导入文件”批量选择文件，也可直接把 Finder 中的文件拖到该入口或文件列表。文件按原名、原格式复制保存，支持文档、压缩包、音视频和图片；暂不接收文件夹或符号链接。导入在后台执行，同名文件分别保存，不覆盖原文件。文件卡片支持命名搜索、置顶、复制、在 Finder 中定位和拖到其他应用；删除或清空只移除站内副本。文件不参与文字/图片的自动剪贴板采集，也不混入其他分类。拖出的文件会在 `~/Movies/悬浮中转站素材/files/` 保留兼容副本，其他项目仍引用这些文件时请勿删除该目录。
...
- **随手收集**：macOS 复制图片或文字后始终进入“待分类”；Windows 进入当前默认分类。
...
以下快捷键和批量操作适用于 Windows；macOS 操作见上面的“macOS 本地构建与运行”。
...
- Windows 动态分类增删、设置界面、快捷启动和常驻模式仍在路线图中，不属于 1.3.0 承诺。macOS 已支持新增类别，暂不支持删除类别。
